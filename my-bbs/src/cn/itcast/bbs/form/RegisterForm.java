...
public class RegisterForm {
    ...
    public boolean validate(User user){
        ...
        if(isValidateUsername(user.getUsername()) && isValidatePassword(user.getPassword())
                && isValidateEmail(user.getEmail()) && isValidatePassword(user.getPassword2())){
            flag=true;
        }
        return flag;
    }   
    public boolean validate2Password(User user){
        boolean flag = false;
     ...
     * 验证二次密码
     */
    private boolean isValidatePassword2(String password){
        boolean flag = false;
        if(password != null && password.trim().length()>0){
            if(password.matches("\\w{6}")){
                flag=true;
            }else{
                errors.put("password2", "<font color='red'>密码必须是6位数字或大小写字母</font>");
            }
        }else{
            errors.put("password2", "<font color='red'>密码不能为空</font>");
        }
        return flag;
    }
    /*
    ...
    private boolean isValidateEmail(String email){
        ...
        if(email != null && email.trim().length()>0){
            if(email.matches("\\w+@\\w+(\\.\\w+)+")){
                ...
                errors.put("email", "<font color='red'>邮箱格式不正确</font>");
            }
        }else{
            errors.put("email", "<font color='red'>邮箱不能为空</font>");
        }
        return flag;
    }
